...
static const char key_word_elements[] = "elements";
static const char key_word_drawArraysTriangles[] = "drawArraysTriangles";
static const char key_word_drawElementsTriangles[] = "drawElementsTriangles";
...
typedef struct {
    int colors_length; // number of color list items
    int positions_length;
    int elements_length;
    // int draw_triangles_length;
    list_item *colors;
    list_item *positions;
    list_item *elements;
    // list_item *draw_triangles;
} pixel_metadata;
...
void free_pixels(pixel_metadata *pixels) {
    ...
    for (int i = 0; i < pixels->elements_length; i++) {
        free(pixels->elements[i].items);
    }
    free(pixels->elements);
...
void increment_vector(vector *v,
                      const float z_s,
                      const float w_s,
                      const float r_s,
                      const float g_s,
...
void scanline(image_t *img, const vector v1, const vector v2) {
    ...
    while (x1 < x2) {
        if (x1 >= 0 && y1 >= 0 && x1 < img->width && y1 < img->height) {
            // const int x1 = roundl(x1);
            // const int y1 = roundl(y1);
            // z_buffer[x1][y1] = z1;
            if (depth) {
...
void s_vector(image_t *img, vector *vectors, const vector v1, const vector v2) {
    // float x1 = viewport_coordinates(img->width, v1.x, v1.w);
    // float y1 = viewport_coordinates(img->height, v1.y, v1.w);
    float x1 = v1.x;
    float y1 = v1.y;
    float z1 = v1.z;
    float w1 = v1.w;
    ...
    float b1 = v1.b;
    float a1 = v1.a;
    // float x2 = viewport_coordinates(img->width, v2.x, v2.w);
    // float y2 = viewport_coordinates(img->height, v2.y, v2.w);
    float x2 = v2.x;
    float y2 = v2.y;
    float z2 = v2.z;
    float w2 = v2.w;
    ...
    if (y1 > y2) {
        x2 = v1.x;
        y2 = v1.y;
        // x2 = viewport_coordinates(img->width, v1.x, v1.w);
        // y2 = viewport_coordinates(img->height, v1.y, v1.w);
        z2 = v1.z;
        w2 = v1.w;
        r2 = v1.r;
        g2 = v1.g;
        b2 = v1.b;
        a2 = v1.a;
        // x1 = viewport_coordinates(img->width, v2.x, v2.w);
        // y1 = viewport_coordinates(img->height, v2.y, v2.w);
        x1 = v2.x;
        y1 = v2.y;
        z1 = v2.z;
        w1 = v2.w;
...
vector find_intersection(const vector v1, const vector v2, const int *p) {
    float v1_d = p[0] * v1.x + p[1] * v1.y + p[2] * v1.z + p[3] * v1.w;
    float v2_d = p[0] * v2.x + p[1] * v2.y + p[2] * v2.z + p[3] * v2.w;

    vector result;
    result.x = (v2_d * v1.x - v1_d * v2.x) / (v2_d - v1_d);
    result.y = (v2_d * v1.y - v1_d * v2.y) / (v2_d - v1_d);
    result.z = (v2_d * v1.z - v1_d * v2.z) / (v2_d - v1_d);
    result.w = (v2_d * v1.w - v1_d * v2.w) / (v2_d - v1_d);
    result.r = (v2_d * v1.r - v1_d * v2.r) / (v2_d - v1_d);
    result.g = (v2_d * v1.g - v1_d * v2.g) / (v2_d - v1_d);
    result.b = (v2_d * v1.b - v1_d * v2.b) / (v2_d - v1_d);
    result.a = (v2_d * v1.a - v1_d * v2.a) / (v2_d - v1_d);
    return result;

    // float t = (p[0] * v1.x + p[1] * v1.y + p[3] * v1.z + p[3]) /
    //         ((p[0] * (v2.x - v1.x)) + (p[1] * (v2.y - v1.y)) + (p[2] * (v2.z - v1.z)));
    // Vertex result;
...
void clip(vector *clipped_vectors, int *clipped_count, vector *original_vectors, int *original_vectors_count, int p) {
    // count will be the index of the original vector as it's looped through
    int *plane = frustum_planes[p];
    int count = 0;
    while (count < *original_vectors_count) {
        int invalid_vertices_count = 0;
        int invalid_vertices[3];
        int valid_vertices_count = 0;
        int valid_vertices[3];
        // clip in intervals of 3
        for (int n = 0; n < 3; n++) {
            float distance = plane[0] * original_vectors[count].x + plane[1] * original_vectors[count].y + plane[2] * original_vectors[count].z + plane[3] * original_vectors[count].w;
            if (distance >= 0.0f) {
                valid_vertices[valid_vertices_count] = count;
                valid_vertices_count++;
            } else {
                invalid_vertices[invalid_vertices_count] = count;
                invalid_vertices_count++;
            }
            count++;
        }
        if (invalid_vertices_count == 3) {
            printf("all vertices invalid\n");
        } else if (invalid_vertices_count == 2) {
            printf("2 vertices invalid\n");
            vector valid_vec = original_vectors[valid_vertices[0]];
            vector invalid_vec_1 = original_vectors[invalid_vertices[0]];
            vector invalid_vec_2 = original_vectors[invalid_vertices[1]];
            vector new_vec_1 = find_intersection(valid_vec, invalid_vec_1, plane);
            vector new_vec_2 = find_intersection(valid_vec, invalid_vec_2, plane);
            clipped_vectors[(*clipped_count)++] = valid_vec;
            clipped_vectors[(*clipped_count)++] = new_vec_1;
            clipped_vectors[(*clipped_count)] = new_vec_2;
            (*clipped_count)++;
        } else if (invalid_vertices_count == 1) {
            printf("1 vertex invalid\n");
            vector valid_vec_1 = original_vectors[valid_vertices[0]];
            vector valid_vec_2 = original_vectors[valid_vertices[1]];
            vector invalid_vec_1 = original_vectors[invalid_vertices[0]];
            vector new_vec_1 = find_intersection(valid_vec_1, invalid_vec_1, plane);
            vector new_vec_2 = find_intersection(valid_vec_2, invalid_vec_1, plane);
            // new triangle 1
            clipped_vectors[(*clipped_count)++] = valid_vec_1;
            clipped_vectors[(*clipped_count)++] = new_vec_1;
            clipped_vectors[(*clipped_count)++] = new_vec_2;
            // new triangle 2
            clipped_vectors[(*clipped_count)++] = valid_vec_1;
            clipped_vectors[(*clipped_count)++] = valid_vec_2;
            clipped_vectors[(*clipped_count)] = new_vec_2;
            (*clipped_count)++;
        } else {
            printf("all vertices valid\n");
            clipped_vectors[(*clipped_count)++] = original_vectors[valid_vertices[0]];
            clipped_vectors[(*clipped_count)++] = original_vectors[valid_vertices[1]];
            clipped_vectors[(*clipped_count)] = original_vectors[valid_vertices[2]];
            (*clipped_count)++;
        }
    }
}
...
void drawArrayTriangles(image_t *img, int first, int count, list_item *positions, list_item *colors) {
    ...
    while (current_pos < count) {
        ...
        for (int k = 0; k < 3; k++) {
            ...
            vectors[k].x = positions->items[start * positions->count];
            vectors[k].y = positions->items[start * positions->count + 1];
        ...
        vector *original_vectors = malloc(1024 * (sizeof(vector) * 3));
        original_vectors[0] = vectors[0];
        original_vectors[1] = vectors[1];
        original_vectors[2] = vectors[2];
        int original_vector_count = 3;

        for (int p = 0; p < FRUSTUM_PLANES; p++) {
            vector *clipped_vectors = malloc(1024 * (sizeof(vector) * 3));
            int clipped_count = 0;
            clip(clipped_vectors, &clipped_count, original_vectors, &original_vector_count, p);
            for (int i = 0; i < clipped_count; i++) {
                original_vectors[i] = clipped_vectors[i];
            }
            original_vector_count = clipped_count;
            free(clipped_vectors);
        }

        // if (clipped_count > 0) {
            int internal_count = 0;
            while (internal_count < original_vector_count) {
                // set up vertices in intervals of 3
                for (int internal_k = 0; internal_k < 3; internal_k++) {
                    original_vectors[internal_count].x = viewport_coordinates(img->width, original_vectors[internal_count].x, original_vectors[internal_count].w);
                    original_vectors[internal_count].y = viewport_coordinates(img->height, original_vectors[internal_count].y, original_vectors[internal_count].w);
                    original_vectors[internal_count].z = original_vectors[internal_count].z/original_vectors[internal_count].w;
                    original_vectors[internal_count].w = 1/original_vectors[internal_count].w;

                    vectors[internal_k] = original_vectors[internal_count];
                    internal_count++;
                }
                // sort 3 vertices
                for (int m = 0; m < 3; m++) {
...
void drawElementsTriangles(image_t *img,
                           const int offset,
                           const int count,
                           const list_item *positions,
                           const list_item *colors,
                           const int *elements) {
    int current_pos = 0;
    while (current_pos < count) {
        vector vectors[3];
        for (int k = 0; k < 3; k++) {
            // int start = current_pos + offset;
            int start = elements[offset + current_pos];

            float z = 0.0f;
            if (positions->count > 2) {
                z = positions->items[start + 2];
            }

            float w = 1.0f;
            if (positions->count > 3) {
                w = positions->items[start + 3];
            }

            // vectors[k].x = viewport_coordinates(img->width, positions->items[start], w);
            // vectors[k].y = viewport_coordinates(img->height, positions->items[start + 1], w);
            float x = positions->items[start];
            float y = positions->items[start + 1];

            float a = 1.0f;
            if (colors->count > 3) {
                a = colors->items[start + 3];
            }
            float r = colors->items[start];
            float g = colors->items[start + 1];
            float b = colors->items[start + 2];
...
int main(int argc, const char **argv) {
    ...
    while (getline(&line, &line_length, file) != -1) {
        if (prefix(key_word_png, line)) {
            ...
            free_tokens(tokens, token_count);
        } else if (prefix(key_word_drawElementsTriangles, line)) {
            char **tokens = malloc(1 * sizeof(char *));
            int token_count = 0;
            tokenize_line(&tokens, &token_count, line, strlen(key_word_drawElementsTriangles), line_length);

            drawElementsTriangles(img, atoi(tokens[1]), atoi(tokens[0]), positions, colors, elements);

            free_tokens(tokens, token_count);
        } else if (prefix(key_word_elements, line)) {
            char **tokens = malloc(1 * sizeof(char *));
            int token_count = 0;
            tokenize_line(&tokens, &token_count, line, strlen(key_word_elements), line_length);

            // elements->items = malloc(token_count * sizeof(float));
            // elements->length = token_count;
            // elements->count = atoi(tokens[0]);
            for (int i = 0; i < token_count; i++) {
                char *pEnd;
                elements[i] = (int) strtof(tokens[i], &pEnd);
                // elements[i] = atoi(tokens[i]);
            }
